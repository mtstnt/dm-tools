...
export function MembersClient({
  ...
  function renderUserRow(user: MemberUser) {
    return (
      <div
        key={user.id}
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-2">View members grouped by team.</p>
        </div>
          ...
          <Card key={team.id} className="flex flex-col">
            ...
            <CardContent className="flex-1">
              {team.users.length === 0 ? (
                ...
                <div className="divide-y">{team.users.map(renderUserRow)}</div>
              )}
            </CardContent>
          </Card>
        ))}
