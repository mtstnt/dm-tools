...
const ORDERED  = ['A1','A2','A3','A4','B1','B2','B3','S1','S2']; // A→B→S output order
const SR_ROLES = ['TC In + Altarcall S1','TC Out + Altarcall S2','FD'];
const EVT_NAMES = ['Teen','Youth'];

// Arrow direction in degrees from UP (clockwise positive)
const ARROW_ROT = {
  S1:90, S2:-90, A1:45, A2:0, A3:-45, A4:-90, B1:0, B2:0, B3:0
};

// SVG Block geometries (viewBox 830 × 490)
const GEOM = {
...
// Fisher-Yates — unbiased true random shuffle
const shuffle = arr => {
  const a = [...arr];
...
const bestPartition = (n, weights) => {
  const opts = getAllAPartitions(Math.min(n, 4));
  if (!opts.length) return [];
  let best = opts[0]; let bestV = Infinity;
  opts.forEach(parts => {
    ...
    if (v < bestV) { bestV=v; best=parts; }
  });
  return best;
};

// ── Auto-assign: 2-zone, max 2 A blocks/person, S1↔S2 exclusive ──
//
// Rules enforced:
//  1. Max 2 A blocks per counter (hard limit)
//  2. Min 2 counters per A block (soft – badge warns if N too small)
//  3. S1 and S2 cannot be assigned to the same person
//  4. B/S blocks only assigned to person whose A blocks are adjacent
//  5. Consecutive A-block rule respected in safety fill
const autoAssignFn = (counters, weights, prev = {}) => {
  if (!counters.length) return {};
  const result = {};
  const loadOf = {};
  counters.forEach(c => { loadOf[c.name] = 0; });

  // prevOf: which blocks each person held in the other event
  const prevOf = {};
  ...
  // Helpers
  const myA  = name => A_BL.filter(b => (result[b]||[]).includes(name));
  const cntA = name => myA(name).length;

  const N  = counters.length;
  const sh = shuffle(counters);

  // ── Phase 1: Fixed 2-zone assignment ──
  // Zone1 = [A1,A2], Zone2 = [A3,A4]  → each person gets exactly 2 A blocks (max-2 satisfied)
  // Odd counter goes to heavier-weighted zone
  ...
  const zCnt = N % 2 === 0
    ? [base, base]
    : (wZ1 >= wZ0 ? [base, base+1] : [base+1, base]);

  // Which zone was each person in during previous event?
  // Used to strongly prefer the opposite zone in the current event.
  const prevZoneOf = name => {
    const pa = (prevOf[name]||[]).filter(b => A_BL.includes(b));
    if (!pa.length) return -1;                          // no prev A-block data
    if (ZONES[0].some(b => pa.includes(b))) return 0;  // was in Zone 0 ([A1,A2])
    if (ZONES[1].some(b => pa.includes(b))) return 1;  // was in Zone 1 ([A3,A4])
    return -1;
  };

  // Shuffle remaining so ties break randomly (array is already shuffled via sh)
  ...
  ZONES.forEach((zone, zi) => {
    for (let k = 0; k < zCnt[zi] && remaining.length; k++) {
      ...
      remaining.forEach((c, i) => {
        ...
        // Block-level overlap with previous event's specific blocks (0–2)
        const ov       = zone.filter(b => (prevOf[c.name]||[]).includes(b)).length;
        // Same zone as before  → heavy penalty (+8): avoid repeat
        const samePen  = prevZone === zi                      ?  8 : 0;
        // Opposite zone        → bonus (−5): ACTIVELY prefer zone-swap
        // No prev data (−1)    → neutral (0)
  ...
  // ── Phase 2: Safety fill — push toward min-2 per A block ──
  // Respects: consecutive rule + MAX 2 A blocks per person
  A_BL.forEach(abl => {
    while ((result[abl]||[]).length < 2) {
      const cands = [...counters]
        .filter(c => !(result[abl]||[]).includes(c.name))
        .filter(c => cntA(c.name) < 2)                    // ← MAX 2 guard
        .filter(c => isConsec([...myA(c.name), abl]))      // ← consecutive guard
        .sort((a, b) => loadOf[a.name] - loadOf[b.name]);
      if (!cands.length) break;  // truly impossible given N — badge will warn
  ...
  // ── Phase 3: B/S blocks ──
  // S1 and S2 are mutually exclusive per person
  BS_BL.forEach(block => {
    const adj = counters.filter(c => myA(c.name).some(a => ADJ[a]?.includes(block)));
    const pool = adj.length ? adj : counters;
    const prevOwners = (Array.isArray(prev[block]) ? prev[block] : [prev[block]]).filter(Boolean);

    // S1 ↔ S2 exclusivity filter
    const eligible = pool.filter(c => {
      if (block === 'S1') return !(result['S2']||[]).includes(c.name);
      if (block === 'S2') return !(result['S1']||[]).includes(c.name);
      return true;
    });
    const finalPool = eligible.length ? eligible : pool; // fallback (very rare edge)

    const sorted = [...finalPool].sort((a,b) => {
      const ap = prevOwners.includes(a.name)?1:0, bp = prevOwners.includes(b.name)?1:0;
...
export default function App() {
  ...
  const [hoveredMbr,  setHoveredMbr]  = useState(null); // member id being hovered
  const [srPicker,    setSRPicker]    = useState(null); // {ei,role} | null
  ...
  /* ── Derived ── */
  const allBlock = useMemo(() => members.slice(0,2), [members]);
  const pool     = useMemo(() => members.slice(2),   [members]);

  // ── Event-specific member lists (respects (T)/(Y) availability) ──
  const eventMembers = useCallback(ei => {
    const label = ei === 0 ? 'teen' : 'youth';
    return members.filter(m => !m.availability || m.availability === 'both' || m.availability === label);
  }, [members]);

  const allBlockForEvent = useCallback(ei => eventMembers(ei).slice(0, 2), [eventMembers]);
  const poolForEvent     = useCallback(ei => eventMembers(ei).slice(2),    [eventMembers]);

  const getSRNames = useCallback(ei => {
    const { tcIn, tcOut, fd } = events[ei].sr;
    return [tcIn,tcOut,fd].filter(Boolean);
  }, [events]);

  const getCounters = useCallback(ei => {
    const sr = getSRNames(ei);
    return poolForEvent(ei).filter(m => !sr.includes(m.name));
  }, [poolForEvent, getSRNames]);

  const getAssignees = useCallback((block, ei=activeEvt) =>
    events[ei].assignments[block] || []
  , [events, activeEvt]);

  // ── Min-2-per-A-block status (current event) ──
  ...
  const getMemberBlocks = useCallback((name, ei=activeEvt) =>
    ORDERED.filter(b => (events[ei].assignments[b]||[]).includes(name))
  , [events, activeEvt]);

  const getMemberRole = useCallback((name, ei=activeEvt) => {
  ...
  const colorOf = useCallback(name =>
    members.find(m=>m.name===name)?.color ?? '#334155'
  , [members]);

  /* ── Member Actions ── */
  const parseMemberStr = raw => {
    ...
    return {
      ...
      availability: tOnly ? 'teen' : yOnly ? 'youth' : 'both',
    };
  };
  ...
  const initialize = () => {
    ...
    // Per-event check: availability tags (T)/(Y) can shrink either event's pool
    const teenN  = eventMembers(0).length;
    const youthN = eventMembers(1).length;
    if (teenN < 5) {
      setError(`Anggota untuk Teen kurang (hanya ${teenN}, minimal 5). Cek tanda (Y) yang membatasi.`); return;
    }
    if (youthN < 5) {
      setError(`Anggota untuk Youth kurang (hanya ${youthN}, minimal 5). Cek tanda (T) yang membatasi.`); return;
    }
    setError('');

    // ── Teen SR: random from Teen-eligible pool ──
    const teenPool  = shuffle(poolForEvent(0));
    const sr1       = teenPool.slice(0, 3);
    const sr1Names  = new Set(sr1.map(m => m.name));

    // ── Youth SR: random from Youth-eligible pool EXCLUDING Teen SR members ──
    ...
    setEvents([
      { assignments:{}, sr:{ tcIn:sr1[0]?.name, tcOut:sr1[1]?.name, fd:sr1[2]?.name } },
      { assignments:{}, sr:{ tcIn:sr2[0]?.name, tcOut:sr2[1]?.name, fd:sr2[2]?.name } },
    ]);
  ...
  const setEventSR = (ei, role, name) => {
    setEvents(prev => prev.map((e, i) =>
      i !== ei ? e : { ...e, sr: { ...e.sr, [role]: name || null } }
    ));
    setSRPicker(null);
  };

  // Members eligible for SR slot (not already holding another SR slot in same event)
  const getSREligible = (ei, excludeRole) => {
    const sr = events[ei].sr;
    const taken = Object.entries(sr)
      .filter(([k]) => k !== excludeRole).map(([,v]) => v).filter(Boolean);
  ...
  const copyOut = async () => {
    try { await navigator.clipboard.writeText(genOutput()); setCopied(true); setTimeout(()=>setCopied(false),2000); }
    catch {}
  };
  ...
  const renderBlock = key => {
    ...
    const stroke = isSelConn ? '#FCD34D'
      : (priColor || (isHov ? '#475569' : '#0F2A4A'));
    const fill = priColor ? priColor+'22'
    ...
    const Shape = g.t==='r'
      ...
      : <polygon points={g.pts.map(([x,y])=>`${x},${y}`).join(' ')}
          fill={fill} stroke={stroke} strokeWidth={sw}/>;
    ...
    return (
      ...
      >
        ...
        <g transform={`translate(${g.cx},${g.cy-22}) rotate(${arRot})`}>
          <polygon points="0,-12 9,-4 5,-4 5,10 -5,10 -5,-4 -9,-4"
            fill="none"
        ...
        {A_BL.includes(key) && (() => {
          ...
          // top-right corner of each A block
          ...
          return (
            <g>
              ...
              {!ok && (
                <circle cx={bx-30} cy={by-2} r="3.5" fill={bC} opacity="0.9"/>
              )}
            </g>
          );
        })()}
  ...
  const renderTooltip = () => {
    ...
    return (
      <g>
        ...
        {/* A-block min-2 status line */}
        {A_BL.includes(hovered) && (() => {
          const cnt = assignees.length;
          const ok  = cnt >= 2;
          const col = ok ? '#10B981' : cnt===1 ? '#F59E0B' : '#EF4444';
          const msg = ok ? `✓ ${cnt}/2 min terpenuhi` : cnt===1 ? '⚠ butuh 1 orang lagi' : '✗ belum ada counter';
          return (
            <g>
  ...
  return (
    <div style={{
      height:'100vh', background:'#050B18', color:'#CBD5E1',
      ...
      <header style={{
        ...
        <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
          ...
          <span style={{
            ...
            letterSpacing:'1.5px', fontFamily:'Syne,sans-serif',
          }}>
            SERVICE ASSIGNMENT
          </span>
        </div>
        ...
        {isReady && (
          <div style={{
            display:'flex', gap:'3px', background:'#0A1929',
            borderRadius:'8px', padding:'3px', border:'1px solid #0A1D35',
          }}>
            ...
            {aStatus.map(({bl, cnt}) => {
              ...
              return (
                <div key={bl} style={{
                  ...
                  background: bBg, border:`1px solid ${bC}44`,
                }}>
                  <span style={{fontSize:'8px', color:bC, fontWeight:700,
                    fontFamily:'DM Mono,monospace', letterSpacing:'1px'}}>
                    {bl}
                  </span>
                  <span style={{fontSize:'8px', color:bC,
        ...
        {isReady && <>
          ...
          <div style={{
            display:'flex', gap:'3px', background:'#0A1929',
            borderRadius:'7px', padding:'3px', border:'1px solid #0A1D35',
          }}>
            {[['view','👁 VIEW'],['manual','✏️ MANUAL']].map(([m,lbl])=>(
              <button key={m}
          ...
          <button onClick={runAuto} style={{
            padding:'6px 14px', background:'#1D4ED8',
          ...
          <button onClick={()=>setShowOut(p=>!p)} style={{
            padding:'6px 12px',
      ...
      <div style={{display:'flex', flex:1, overflow:'hidden'}}>
        ...
        <aside style={{
          ...
          <div style={{padding:'12px', borderBottom:'1px solid #0A1929'}}>
            <div style={{display:'flex', gap:'6px', marginBottom:'8px'}}>
              ...
              <button onClick={addSingle} style={{
                width:'34px', height:'34px', background:'#1D4ED8', border:'none',
                borderRadius:'6px', color:'white', fontSize:'18px',
                cursor:'pointer', fontWeight:700, lineHeight:1, flexShrink:0,
              }}>+</button>
            </div>
            ...
            {showImport && (
              <div style={{marginTop:'8px'}}>
                <div style={{
                  fontSize:'8px', color:'#1E3A5F', marginBottom:'5px',
                ...
                <textarea value={bulkText} onChange={e=>setBulkText(e.target.value)}
                  ...
                  rows={5}
                  style={{
                    width:'100%', background:'#0A1929', border:'1px solid #0F2A4A',
                    borderRadius:'6px', padding:'8px', color:'#CBD5E1',
                    fontSize:'11px', resize:'none', outline:'none',
                    fontFamily:'DM Mono,monospace', lineHeight:1.8,
                  }}/>
                <button onClick={parseBulk} style={{
                  width:'100%', marginTop:'4px', padding:'6px',
                  background:'#0F2A4A', border:'1px solid #1E3A5F',
                  borderRadius:'6px', color:'#64748B', fontSize:'10px',
                  cursor:'pointer', fontFamily:'DM Mono,monospace', letterSpacing:'1px',
                }}>
          ...
          {/* Min-2 rule info strip */}
          {isReady && (
            <div style={{
              padding:'5px 12px', borderBottom:'1px solid #0A1929',
              background:'#040D1A', display:'flex', alignItems:'center', gap:'8px',
            }}>
              <span style={{fontSize:'7.5px', color:'#0F2A4A',
                letterSpacing:'1px', fontFamily:'DM Mono,monospace'}}>
                MIN 2 ORG/BLOK A
              </span>
              <span style={{
                fontSize:'7.5px', fontWeight:700, marginLeft:'auto',
                color: allAMet ? '#10B981' : '#F59E0B',
                fontFamily:'DM Mono,monospace', letterSpacing:'0.5px',
              }}>
          ...
          <div style={{flex:1, overflowY:'auto', padding:'8px'}}>
            {members.length===0 ? (
              <div style={{
                textAlign:'center', color:'#0F2A4A', padding:'30px 10px',
                fontSize:'10px', lineHeight:2.5, fontFamily:'DM Mono,monospace',
                letterSpacing:'1px',
              }}>
                BELUM ADA ANGGOTA<br/>
                Tambah di atas atau import
              </div>
            ) : (
              <>
                <div style={{
                  fontSize:'8px', color:'#0F2A4A', marginBottom:'8px',
                  padding:'0 4px', fontFamily:'DM Mono,monospace', letterSpacing:'1px',
                }}>
                  {members.length} ANGGOTA · 2 TERATAS = KOORDINATOR ALL BLOCK
                </div>

                {members.map((m, i) => {
                  // Availability check for current event
                  ...
                  // Compute ALL BLOCK label across both events
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{
                          ...
                          padding:'8px 8px 6px',
                          cursor: mode==='manual'&&isCounter?'pointer':'default',
                        }}
                      >
                        <div style={{
                          width:'10px', height:'10px', borderRadius:'50%', flexShrink:0,
                          background:m.color, boxShadow:`0 0 7px ${m.color}77`,
                        }}/>
                        <div style={{flex:1, minWidth:0}}>
                          <div style={{display:'flex',alignItems:'center',gap:'5px',flexWrap:'wrap'}}>
                            ...
                            {avail!=='both'&&<span style={{fontSize:'7px',padding:'1px 5px',borderRadius:'3px',background:avail==='teen'?'#78350F55':'#1E3A8A55',color:avail==='teen'?'#FBBF24':'#93C5FD',fontFamily:'DM Mono,monospace',letterSpacing:'1px',border:'1px solid '+(avail==='teen'?'#FBBF2444':'#93C5FD44')}}>{avail==='teen'?'T':'Y'}</span>}
                          </div>
                      ...
                      {isHov && (
                        <div style={{
                          ...
                          borderTop:'1px solid #0F2A4A', background:'#040D1A',
                        }}>
                          <button
                            ...
                            <span style={{fontSize:'8px',letterSpacing:'0.5px'}}>NAIK</span>
                          </button>
                          <button
                            onClick={e=>{e.stopPropagation();moveMember(m.id,1);}}
                            ...
                            style={{flex:1,padding:'5px 4px',background:'#0A1929',border:'1px solid #0F2A4A',borderRadius:'5px',color:'#475569',fontSize:'10px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'3px'}}>
                            <span style={{fontSize:'12px'}}>↓</span>
                            <span style={{fontSize:'8px',letterSpacing:'0.5px'}}>TURUN</span>
                          </button>
                          <button
                            ...
                            <span style={{fontSize:'12px'}}>×</span>
                            <span style={{fontSize:'8px',letterSpacing:'0.5px'}}>HAPUS</span>
                          </button>
                        </div>
                      )}
                    </div>
          ...
          {isReady && (() => {
            const SR_KEYS  = ['tcIn','tcOut','fd'];
            const SR_SHORT = ['TC In + AC S1','TC Out + AC S2','FD'];
            const SR_FULL  = ['TC In + Altarcall S1','TC Out + Altarcall S2','FD'];
                    ...
                    return (
                      <div key={key} style={{position:'relative'}}>
                        ...
                        <div style={{
                          ...
                          background: isOpen ? '#0A1929' : '#060F1E',
                          border:'1px solid '+(isOpen?'#1D4ED855':'#0F2A4A'),
                          cursor:'pointer',
                        }}
                        ...
                        {isOpen && (
                          <div style={{
                            ...
                            background:'#060F1E', border:'1px solid #1D4ED855',
                            borderTop:'none', borderRadius:'0 0 6px 6px',
                            maxHeight:'140px', overflowY:'auto', boxShadow:'0 8px 24px #000A',
                          }}>
                            {/* Clear option */}
                            <div
                            ...
                            {eligible.length === 0 && (
                              <div style={{padding:'8px 10px',fontSize:'9px',color:'#1E3A5F',fontFamily:'DM Mono,monospace'}}>
                                Tidak ada anggota tersedia
                              </div>
                            )}
                            {eligible.map(m => (
                              <div key={m.id}
                                onClick={()=>setEventSR(activeEvt, key, m.name)}
                                style={{
                                  display:'flex', alignItems:'center', gap:'8px',
                                  padding:'7px 10px', cursor:'pointer',
                                  background: assigned===m.name ? '#0A1929' : 'transparent',
                                  borderBottom:'1px solid #0A1929',
                                }}
                                onMouseEnter={e=>e.currentTarget.style.background='#0A1929'}
                                onMouseLeave={e=>e.currentTarget.style.background=assigned===m.name?'#0A1929':'transparent'}
                              >
                                <div style={{width:'7px',height:'7px',borderRadius:'50%',background:m.color,boxShadow:`0 0 5px ${m.color}66`,flexShrink:0}}/>
                                <span style={{fontSize:'11px',fontWeight:700,color:'#E2E8F0'}}>{m.name}</span>
                                {assigned===m.name && <span style={{marginLeft:'auto',fontSize:'8px',color:'#10B981'}}>✓ terpilih</span>}
                              </div>
                            ))}
                          </div>
                        )}
          ...
          <div style={{padding:'10px', borderTop:'1px solid #0A1929'}}>
            {error && (
              <div style={{
                background:'#1C0909', border:'1px solid #7F1D1D44',
                borderRadius:'6px', padding:'8px', fontSize:'9px',
                color:'#FCA5A5', marginBottom:'8px', lineHeight:1.6,
                fontFamily:'DM Mono,monospace',
              }}>
                ⚠ {error}
              </div>
            ...
            {mode==='manual' && isReady && (
              <div style={{
                background:'#1A1200', border:'1px solid #78350F55',
                borderRadius:'6px', padding:'8px', fontSize:'9px',
                color:'#FDE68A', marginBottom:'8px', lineHeight:1.7,
                fontFamily:'DM Mono,monospace',
              }}>
                {selected ? (
                  <>
            ...
            {!isReady ? (
              ...
              <div style={{display:'flex',gap:'6px'}}>
                <button onClick={initialize} style={{
                  flex:1, padding:'7px 4px', background:'#0A1929',
                  border:'1px solid #0F2A4A', borderRadius:'6px',
                  color:'#334155', fontSize:'9px', cursor:'pointer',
                  fontFamily:'DM Mono,monospace', letterSpacing:'0.5px',
                }}>🎲 RE-ROLL SR</button>
        ...
        <main style={{flex:1, display:'flex', flexDirection:'column', overflow:'hidden'}}>
          ...
          {/* SVG Map */}
          <div style={{flex:1, overflow:'hidden', background:'#030810', position:'relative'}}>
            {!isReady ? (
              <div style={{
              ...
              <svg viewBox="0 0 830 490"
                ...
                {/* Connecting lines from A blocks to main stage */}
                {A_BL.map(key=>{
                  const g=GEOM[key];
                  const assignees=getAssignees(key,activeEvt);
          ...
          {showOut && isReady && (
            <div style={{
              height:'215px', background:'#07111E',
              borderTop:'1px solid #0A1929', display:'flex', flexDirection:'column',
            }}>
              <div style={{
                display:'flex', alignItems:'center', justifyContent:'space-between',
                padding:'8px 16px', borderBottom:'1px solid #0A1929',
              }}>
                <span style={{
                  fontSize:'9px', fontWeight:600, color:'#1E3A5F',
                  fontFamily:'DM Mono,monospace', letterSpacing:'1.5px',
                }}>OUTPUT TEKS</span>
                <button onClick={copyOut} style={{
                  padding:'4px 12px',
